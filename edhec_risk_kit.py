...
def bond_cash_flows(maturity, principal=100, coupon_rate=0.03, coupons_per_year=12):
    ...
    cash_flows.iloc[-1] += principal  # add the principal repayment to the last cash flow at maturity
    return cash_flows
...
def glidepath_allocator(r1, r2, start_glide=1, end_glide=0.0):
    ...
    by gradually moving from start_glide to end_glide over time, in the style of a target date fund
    """
    n_points = r1.shape[0]
    n_col = r1.shape[1]
...
def drawdown_allocator(psp_r, ghp_r, maxdd, m=3):
    """
    Allocate between PSP and GHP with the goal to provide exposure to the upside
    of the PSP without going violating the floor. Instead of a constant floor, we have the drawdown.
    Uses a CPPI-style dynamic risk budgeting algorithm by investing a multiple
    of the cushion in the PSP
    Returns a DataFrame with the same shape as the psp/ghp representing the weights in the PSP
    """
